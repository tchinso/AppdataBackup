...
- `Local`은 제외 목록에 없는 바로 아래 폴더 중 설정한 크기 이하인 폴더와 바로 아래의 일반 파일을 백업합니다.
- `Local` 제외 폴더는 설정 창에서 한 줄에 하나씩 지정하며, 폴더 이름은 대소문자를 구분하지 않습니다.
...
- AppData Local 제외 폴더: `ConnectedDevicesPlatform`, `NVIDIA`
